// 1. Load your Landsat 7 image from Assets
var assetImage = ee.Image("projects/ee-sahlafathima981/assets/clipmosaic_band__stack_raster");

// 2. Rename bands to match Landsat 7 convention (optional but useful for consistency)
var image = assetImage.select(
  ['b1', 'b2', 'b3', 'b4', 'b5', 'b6'],
  ['B1', 'B2', 'B3', 'B4', 'B5', 'B7']
);

// 3. Apply scaling if needed (Landsat SR usually needs scale factor 0.0000275 and offset -0.2)
// But if your image is preprocessed, you can skip this
var scaled = image.multiply(0.0000275).add(-0.2);

// 4. Clip to ROI (if needed)
var clipped = scaled.clip(roi);

// 5. Display the image
Map.centerObject(clipped, 9);
Map.addLayer(clipped, {bands: ['B4', 'B3', 'B2'], min: 0, max: 0.3}, 'Your Asset Image');

// 6. Print bands
print('Bands:', clipped.bandNames());

// Now you can use `filled` image for classification
Map.centerObject(roi, 10);

// Visualize
Map.addLayer(image, { min: [0.1, 0.05, 0.05], max: [0.4, 0.3, 0.2], bands: ['B5', 'B4', 'B7']}, 'Image');
// Enhanced Visualization Layers for All Classes
var visParams = {
  'True Color': { bands: ['B3', 'B2', 'B1']},
  'False Color': { bands: ['B4', 'B3', 'B2']},
  'SWIR Composite': { bands: ['B5', 'B4', 'B3']},
  'Urban Composite': { bands: ['B7', 'B5', 'B3']}, // Built-up areas
  'Water Composite': { bands: ['B2', 'B4', 'B5']}     // Water-related features
};
...
var columns = ['B1', 'B2', 'B3', 'B4', 'B5', 'B7', 'NDVI', 'EVI', 'SAVI', 'MNDWI', 'AWEI', 'NDMI', 'NDWI', 'NDBI', 'NDBaI', 'UI', 'BSI', 'GNDVI', 'elevation', 'classvalue', 'sample'];
var features = ['B1', 'B2', 'B3', 'B4', 'B5', 'B7', 'NDVI', 'EVI', 'SAVI', 'MNDWI', 'AWEI', 'NDMI', 'NDWI', 'NDBI', 'NDBaI', 'UI', 'BSI', 'GNDVI', 'elevation'];
...
Export.table.toDrive({
  ...
  fileFormat: 'CSV',
  selectors: columns,
  description: 'Samples_LC_SA_2004',
  folder: 'LULC_2004_RF'
});
...
Export.image.toDrive({
    image: lc,
    description: 'LULC_Classification_2004_RF',
    folder: 'LULC_2004_RF', // Change the folder name as needed
...
Export.table.toDrive({
  ...
  folder: 'LULC_2004_RF',
  fileNamePrefix: 'classification_metrics_2004',
  fileFormat: 'CSV',
